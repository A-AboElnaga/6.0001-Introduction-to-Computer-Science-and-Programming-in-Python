...
def get_word_score(word,n):
    ...
    for letter in L_word:
        ...
        single_word_sum += letter_value
    second_component = ((7*length)-(3*(n-length)))
    if second_component > 1:
        score = single_word_sum * second_component
    else:
        score = single_word_sum
    return score
...
def update_hand(hand, word):
    ...
    word = word.lower()
    for key in word:
           if key in hand and new_hand[key] >= 1:
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list:
        for letter in word:
            if letter in hand and new_hand[letter] >= 1:
               new_hand[letter] = new_hand.get(letter) - 1
               result.append(True)
            elif letter in hand and new_hand[letter] <= 0:
               result.append(False)
        ...
        for vowel in VOWELS:
            wild_word = word.replace("*", vowel)
            if wild_word in word_list:
                result_wild_word.append(True)
            elif wild_word not in word_list:
                result_wild_word.append(False)
        if True in result_wild_word:
            result.append(True)
        else:
            result.append(False)
    else:
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    HAND_SIZE = input("Enter hand size: ")
    n= int(HAND_SIZE)
    while Num_hands > 0:
        hand = deal_hand(n)
        display_hand(hand)
        Sub_Dec = input("Do you want to substitute a letter ? ")
        Sub_Dec = Sub_Dec.lower()
        new_hand = hand.copy()
        if Sub_Dec == "yes":
